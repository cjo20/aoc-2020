...
int grid_index(int w, int h, int r, int c)
{
    return w * DIM * DIM * DIM + h * DIM * DIM + r * DIM + c;
}

int count_neighbours_active(char * grid, int w, int h, int r, int c)
{
    int num_active = 0;
    ...
    {    
        for (int hh = h - 1; hh <= h + 1; hh++)
        {
            for (int rr = r - 1; rr <= r + 1; rr++)
            {
                ...
                {
                    if (w == ww && h == hh && r == rr && c == cc) continue;
                    num_active += grid[grid_index(ww, hh, rr, cc)];
                }
            }
        }
    }
...
{
    char * grid_copy = (char *) malloc(DIM * DIM * DIM * DIM * sizeof(char));

    memcpy(grid_copy, grid, DIM*DIM*DIM*DIM*sizeof(char));
    for (int ww = current_offset; ww < current_width + current_offset; ww++)
    {    
        for (int hh = current_offset; hh < current_width + current_offset; hh++)
        {
            for (int rr = current_offset; rr < current_width + current_offset; rr++)
            {
                ...
                {
                    int active_neighbours = count_neighbours_active(grid_copy, ww, hh, rr, cc);
                    if (grid_copy[grid_index(ww, hh, rr, cc)])
                    {
                    ...
                    if (!grid_copy[grid_index(ww, hh, rr, cc)])
                    {
                        if (active_neighbours == 3) grid[grid_index(ww, hh, rr, cc)] = 1;;
                    }
                }
            }
        }
    ...
    {    
        for (int hh = current_offset; hh < current_width + current_offset; hh++)
        {
            for (int rr = current_offset; rr < current_width + current_offset; rr++)
            {
                for (int cc = current_offset; cc < current_width + current_offset; cc++)
                {
                    active += grid[grid_index(ww, hh, rr, cc)];
                }
            }
        }
    }
...
void print_grid(char * grid, int current_width, int current_offset)
{
    for (int hh = current_offset; hh < current_width + current_offset; hh++)
    {
        ...
        for (int rr = current_offset; rr < current_width + current_offset; rr++)
        {
            for (int cc = current_offset; cc < current_width + current_offset; cc++)
            {
                printf("%d", grid[grid_index(0, hh, rr, cc)]);
            }
            printf("\n");
        }
    }   
...
int main(void)
{
    ...
    char * grid = (char *) calloc(DIM * DIM * DIM * DIM, sizeof(char));
    int initial_width = 0;
  ...
  //  {
        for(int r = initial_offset; r < initial_width + initial_offset; ++r)
        {
            for( int c = initial_offset; c < initial_width + initial_offset; ++c)
            {
                ...
                if (inp == '.') grid[grid_index(initial_offset, initial_offset, r, c)] = 0;
                else grid[grid_index(initial_offset, initial_offset, r, c)] = 1;
            }
            printf("Done row %d\n", r);
            fgetc(input);
